package ja.ac.it_college.std.s22022.databasesample
...
import ja.ac.it_college.std.s22022.databasesample.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun loadCocktailMemos(): String {
        ...
        val select = """
            ...
            | WHERE _id = ?
        """.trimMargin()
        // INSERT SQL 文を実行してカーソルオブジェクトを取得
        val cursor = db.rawQuery(select, arrayOf("$_cocktailId"))

        // カーソルからデータを取り出す。なければ空白文字列("")を返す
        return cursor.use {
            if (it.moveToNext()) {
